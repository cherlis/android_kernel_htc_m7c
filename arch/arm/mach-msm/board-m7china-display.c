...
#include "../../../../drivers/video/msm412/msm_fb.h"
#include "../../../../drivers/video/msm412/mipi_dsi.h"
#include "../../../../drivers/video/msm412/mdp4.h"
#else
...
#if defined(CONFIG_MACH_M7C_DTU) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY)
...
static int m7china_detect_panel(const char *name)
{
#if 0
...
static struct msm_fb_platform_data msm_fb_pdata = {
	.detect_client = m7china_detect_panel,
};
...
void __init m7china_allocate_fb_region(void)
{
	void *addr;
	unsigned long size;
...
#endif
void __init m7china_mdp_writeback(struct memtype_reserve* reserve_table)
{
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		rc = regulator_set_voltage(reg_l22, 3000000, 3000000);
		if (rc) {
...
{
	if (first_init) {
		PR_DISP_INFO("%s: Skip workaround for first panel init sequence\n", __func__);
		goto done;
	}

	if (panel_type == PANEL_ID_M7CHINA_SHARP_NT) {
...
static struct mipi_dsi_panel_platform_data *mipi_m7china_pdata;

static struct dsi_buf m7china_panel_tx_buf;
static struct dsi_buf m7china_panel_rx_buf;
...
void m7china_set_cabc (struct msm_fb_data_type *mfd, int mode);
#endif
...
static struct dsi_cmd_desc sharp_nt_display_on_cmds[] = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(display_on), display_on},
};

static struct dsi_cmd_desc sharp_nt_display_off_cmds[] = {
...
static int m7china_lcd_on(struct platform_device *pdev)
{
...
static int m7china_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static int __devinit m7china_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_m7china_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static void m7china_display_on(struct msm_fb_data_type *mfd)
{
...
static void m7china_display_off(struct msm_fb_data_type *mfd)
{
...
#ifdef CABC_DIMMING_SWITCH
static void m7china_dim_on(struct msm_fb_data_type *mfd)
{
...
static unsigned char m7china_shrink_pwm(int val)
{
...
static void m7china_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if ((panel_type == PANEL_ID_M7_JDI_SAMSUNG) ||
		...
		samsung_ctrl_brightness[1] = m7china_shrink_pwm((unsigned char)(mfd->bl_level));
	else if ((panel_type == PANEL_ID_M7_SHARP_RENESAS_C1) ||
		 (panel_type == PANEL_ID_M7CHINA_SHARP_NT))
		display_brightness[1] = m7china_shrink_pwm((unsigned char)(mfd->bl_level));
	else
	...
	if (cabc_mode == 3) {
		m7china_set_cabc(mfd, cabc_mode);
	}
#endif
...
static void m7china_color_enhance(struct msm_fb_data_type *mfd, int on)
{
...
static void m7china_sre_ctrl(struct msm_fb_data_type *mfd, unsigned long level)
{
...
static struct platform_driver this_driver = {
	.probe  = m7china_lcd_probe,
	.driver = {
		.name   = "mipi_m7china",
	},
};

static struct msm_fb_panel_data m7china_panel_data = {
	.on	= m7china_lcd_on,
	.off	= m7china_lcd_off,
	.set_backlight = m7china_set_backlight,
	.display_on = m7china_display_on,
	.display_off = m7china_display_off,
	.color_enhance = m7china_color_enhance,
#ifdef CABC_DIMMING_SWITCH
	.dimming_on = m7china_dim_on,
#endif
#ifdef CONFIG_FB_MSM_CABC_LEVEL_CONTROL
	.set_cabc = m7china_set_cabc,
#endif
	.sre_ctrl = m7china_sre_ctrl,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0};

static int mipi_m7china_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_m7china", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	m7china_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &m7china_panel_data,
		sizeof(m7china_panel_data));
	if (ret) {
...
static int __init mipi_cmd_jdi_renesas_init(void)
{
	...
	ret = mipi_m7china_device_register(&pinfo, MIPI_DSI_PRIM,
...
static int __init mipi_command_samsung_init(void)
{
	...
	m7china_panel_data.esd_workaround = true;
#endif
...
static int __init mipi_cmd_sharp_nt_720p_pt_init(void)
{
	int ret;
	...
	ret = mipi_m7china_device_register(&pinfo, MIPI_DSI_PRIM, MIPI_DSI_PANEL_WVGA_PT);
	...
	display_on_cmds = sharp_nt_display_on_cmds;
	display_on_cmds_count = ARRAY_SIZE(sharp_nt_display_on_cmds);
	display_off_cmds = sharp_nt_display_off_cmds;
	display_off_cmds_count = ARRAY_SIZE(sharp_nt_display_off_cmds);
	backlight_cmds = sharp_cmd_backlight_cmds;
	backlight_cmds_count = ARRAY_SIZE(sharp_cmd_backlight_cmds);
...
void __init m7china_init_fb(void)
{
...
static int __init m7china_panel_init(void)
{
	int ret;
	...
	mipi_dsi_buf_alloc(&m7china_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&m7china_panel_rx_buf, DSI_BUF_SIZE);
	...
	} else if (panel_type == PANEL_ID_M7CHINA_SHARP_NT) {
		mipi_cmd_sharp_nt_720p_pt_init();
		PR_DISP_INFO("%s panel ID = PANEL_ID_M7CHINA_SHARP_NT\n", __func__);
...
late_initcall(m7china_panel_init);
